...
use App\Models\coach;
    ...
    $coach = coach::where('name' , auth()->user()->name)->first();
    $team =  $coach->team
    // dd($coach->team);
@endphp
<!DOCTYPE html>
...
@if (!$team->squadlist()->where('match_id',  $match->id)->get()->first())
    
@if ($match->teams()->first()->id == $coach->team->id || $match->teams()->skip(1)->first()->id == $coach->team->id)
@php
$team1 = $match->teams()->first();
$team2 = $match->teams()->skip(1)->first();
@endphp     
                    <div class="match">
                        <a style="text-decoration: none; color:black;" href="{{ route('squadlistForm',[$match , $coach]) }}">
