...
<section class="matches" id="matches">
    <div class="container">
        <h1 class="section-heading" style="text-align: center;">invalid matches results</h1>
...
@endphp     
@if ($match->result)
    {{-- @dd($match->result->isValid) --}}
        @if ($match->result->isValid == "not valid")
                
        <div class="match">
                            
            <a style="text-decoration: none; color:black;" href="admin/results/{{ $match->result->id }}/edit">
        ...
        @endauth
        <div class="alert">
            <h4 class="alert-title">{{$match->result->isValid}}</h4>
            <p class="alert-message">{{$match->result->message}}</p>
        </div>
        </a>
